...
def get_market_days(start_year, end_year, exchange='NYSE'):
    ...
    schedule = market_calendar.schedule(start_date=start_date_str, end_date=end_date_str)
...
def versions(i, stock, stock_data, RESET=False):
    ...
    if i == 1:
        ...
        def parallel_v1_optimize_model(data, train_split=0.7, total_trials=16, num_workers=4):
            previous_params = opt_history.get_parameters(stock, 1)
            storage_url = "sqlite:///study_v1.db"
            study_name = f"study_v1_{stock}"
            ...
            def worker(n_trials):
                local_study = optuna.load_study(study_name=study_name, storage=storage_url)
                local_study.optimize(v1_objective, n_trials=n_trials)
            
            trials_per_worker = total_trials // num_workers
            processes = []
            for _ in range(num_workers):
                p = mp.Process(target=worker, args=(trials_per_worker,))
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
        ...
        def v1_optimize_model(data, train_split=0.7):
            return parallel_v1_optimize_model(data, train_split, total_trials=16, num_workers=4)
        ...
        def v1_main():
            ...
            with tf.device(device):
                model = v1_build_lstm(X_train.shape[1:], units=best_params['units'],
    ...
    # -----------------------------
        ...
        def parallel_v2_optimize_model(data, train_split=0.7, total_trials=16, num_workers=4):
            previous_params = opt_history.get_parameters(stock, 2)
            storage_url = "sqlite:///study_v2.db"
            study_name = f"study_v2_{stock}"
            study = optuna.create_study(
                study_name=study_name,
                direction="minimize",
                storage=storage_url,
            ...
            def worker(n_trials):
                local_study = optuna.load_study(study_name=study_name, storage=storage_url)
                local_study.optimize(v2_objective, n_trials=n_trials)
        ...
        def v2_optimize_model(data, train_split=0.7):
            return parallel_v2_optimize_model(data, train_split, total_trials=16, num_workers=4)
        ...
        def v2_main():
            ...
            with tf.device(device):
                model = v2_build_lstm(X_train.shape[1:], units=best_params['units'],
                                      dropout_rate=best_params['dropout_rate'],
                                      learning_rate=best_params['learning_rate'])
                early_stop = EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True)
                model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=50, batch_size=32, callbacks=[early_stop])
    ...
    # -----------------------------
        ...
        def parallel_v3_optimize_model(data, train_split=0.7, total_trials=16, num_workers=4):
            previous_params = opt_history.get_parameters(stock, 3)
            storage_url = "sqlite:///study_v3.db"
            study_name = f"study_v3_{stock}"
            study = optuna.create_study(
                study_name=study_name,
                direction="minimize",
                storage=storage_url,
            ...
            def worker(n_trials):
                local_study = optuna.load_study(study_name=study_name, storage=storage_url)
                local_study.optimize(v3_objective, n_trials=n_trials)
        ...
        def v3_optimize_model(data, train_split=0.7):
            return parallel_v3_optimize_model(data, train_split, total_trials=16, num_workers=4)
    ...
    # -----------------------------
        ...
        def parallel_v4_optimize_model(data, train_split=0.7, total_trials=16, num_workers=4):
            previous_params = opt_history.get_parameters(stock, 4)
            storage_url = "sqlite:///study_v4.db"
            study_name = f"study_v4_{stock}"
            study = optuna.create_study(
                study_name=study_name,
                direction="minimize",
                storage=storage_url,
            ...
            def worker(n_trials):
                local_study = optuna.load_study(study_name=study_name, storage=storage_url)
                local_study.optimize(v4_objective, n_trials=n_trials)
        ...
        def v4_optimize_model(data, train_split=0.7):
            return parallel_v4_optimize_model(data, train_split, total_trials=16, num_workers=4)
    ...
    # -----------------------------
        ...
        def parallel_v5_optimize_model(data, train_split=0.7, total_trials=16, num_workers=4):
            previous_params = opt_history.get_parameters(stock, 5)
            storage_url = "sqlite:///study_v5.db"
            study_name = f"study_v5_{stock}"
            study = optuna.create_study(
                study_name=study_name,
                direction="minimize",
                storage=storage_url,
            ...
            def worker(n_trials):
                local_study = optuna.load_study(study_name=study_name, storage=storage_url)
                local_study.optimize(v5_objective, n_trials=n_trials)
        ...
        def v5_optimize_model(data, train_split=0.7):
            return parallel_v5_optimize_model(data, train_split, total_trials=16, num_workers=4)
    ...
    # -----------------------------
        ...
        def parallel_v6_optimize_model(data, train_split=0.7, total_trials=16, num_workers=4):
            previous_params = opt_history.get_parameters(stock, 6)
            storage_url = "sqlite:///study_v6.db"
            study_name = f"study_v6_{stock}"
            study = optuna.create_study(
                study_name=study_name,
                direction="minimize",
                storage=storage_url,
            ...
            def worker(n_trials):
                local_study = optuna.load_study(study_name=study_name, storage=storage_url)
                local_study.optimize(v6_objective, n_trials=n_trials)
        ...
        def v6_optimize_model(data, train_split=0.7):
            return parallel_v6_optimize_model(data, train_split, total_trials=16, num_workers=4)
