    ...
    if i == 1:
        ...
        def v1_optimize_model(data, train_split=0.7):
            ...
            def v1_objective(trial):
                ...
                K.clear_session()  # Clear resources used by the model
                return val_loss
        ...
        def v1_future_predictions_method_2(model, last_sequence, future_days):
            ...
            for _ in range(future_days+1):
                ...
                # Instead of np.append, shift the batch and insert new prediction:
        ...
        def v1_main():
            ...
            # Clear Keras session after finishing work for version 1
            K.clear_session()
...
def process_stocks(stock):
    ...
    for dates_index, dates_ in enumerate(full_dates, start=1):
        ...
        with multiprocessing.Pool(processes=int(processes_num/2)) as pool:
