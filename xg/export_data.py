...
def export_optimization_history_to_json():
    ...
    # Fix: Correct sorting of numeric and string versions
    sorted_history_data = {
        stock: {
            k: history_data[stock][k]
            for k in sorted(history_data[stock], key=lambda x: (x.isdigit(), x if x.isdigit() else x.lower()))
        }
