...
def v5_get_market_days(start_year, end_year, exchange='NYSE'):
...
def v5_fetch_technical_data(stock, start_date, end_date, technical_indicators=['rsi']):
    df_list = []
...
def v5_optimize_model(data, train_split=0.7):
    def v5_objective(trial):
        ...
        X, y = v5_create_dataset(data, time_step)
        ...
        model = v5_build_lstm(X_train.shape[1:], units, dropout_rate, learning_rate, decay_factor)
    ...
    study.optimize(v5_objective, n_trials=50)
    
        # Convergence Analysis: Visualize improvement
...
def v5_plot_predictions(stock_data, future_dates, methods, labels, last_date, future_data, stock):
...
def v5_main():
    ...
    market_days = v5_get_market_days(2015, 2025)
    print()
    
    #start
    stock_data = v5_fetch_technical_data(stock, start_date, end_date, technical_indicators)
    ...
    if plot:
        ...
        future_data = v5_fetch_technical_data(stock, last_date, future_date)
        
        v5_plot_predictions(stock_data, future_dates, [future_prices_method_1, future_prices_method_2], ['Method 1', 'Method 2'], stock_data.index[-1], future_data, stock)
